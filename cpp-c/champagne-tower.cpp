...
/*used memoization(dp) + recursion*/
class Solution {
public:
    double t[101][101]; 
    double recsolve(int poured , int i , int j){
        if(i<0 || j <0 || i<j)return 0.0 ;
        if(i ==0 && j==0) return poured ;
        if(t[i][j] != -1) return t[i][j];
        double left = (recsolve(poured ,i-1 , j-1) - 1)/2.0;
        if(left<0) left = 0.0;
        double right = (recsolve(poured ,i-1 , j)-1)/2.0;
    ...
    double champagneTower(int poured, int query_row, int query_glass) {
        ...
        return min(1.0 , recsolve(poured , query_row , query_glass));
    }
};
